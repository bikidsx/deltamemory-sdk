...
const sampleQuestions = [
  "What are the key terms of the MSA?",
  "When did DataSync first breach the contract?",
  "What did Sarah Chen say about the missed deadline?",
  "Find contradictions in the testimony",
  "Build a timeline of events",
  "What damages is TechFlow claiming?",
];

export function Chat({ caseData, onToolLatency }: ChatProps) {
  ...
  return (
    <div className="flex flex-col h-full border border-[var(--border)] rounded-lg">
      <div className="px-4 py-2 border-b border-[var(--border)] flex-shrink-0">
        <span className="font-medium">{caseData.shortName}</span>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {sampleQuestions.map((q) => (
                <button
                  ...
                  className="px-3 py-1.5 text-xs rounded-full border border-[var(--border)] hover:border-[var(--accent)] hover:bg-[var(--accent)]/5 transition-colors"
                >
                  {q}
                </button>
              ))}
                ...
                if (part.type === "tool-recallMemory" || part.type === "tool-storeMemory") {
                  ...
                  return (
                    <span key={i} className="text-xs text-[var(--text-muted)] italic block">
                      {part.type === "tool-recallMemory" ? "🔍 searching case files..." : "💾 noting..."}
                    </span>
                  );
                }
                return null;
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-[var(--border)] flex-shrink-0">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask about contracts, depositions, timeline..."
